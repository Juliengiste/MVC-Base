...
class Personne extends Metier {
	...
	public function isConnected($pmanager = false) {
		...
		elseif (isset($_POST['login_login']) && (isset($_POST['login_pwd']))) {
			...
			$data = array(
				'secret' => '6LdCGnEUAAAAAI3e9GTp2FP_VQyiKQnrXMFCwjP_',
			...
			$captcha_count = (isset($_SESSION['reCaptcha-wrong-count'][session_id()])) ? $_SESSION['reCaptcha-wrong-count'][session_id()] : 0;
			if ($captcha_success->success == true) {
				if (!isset($pmanager)) {
					Utils::handleError('Personne::isConnected essaie de connexion sans Manager', 'E_CORE_NOTICE');
				}
				elseif (!isset($captcha_count) || isset($captcha_count) && $captcha_count <= MAXCOUNTLOGIN) {
					unset($_SESSION['reCaptcha-wrong-count'][session_id()]);
					return $pmanager->login($_POST['login_login'], $_POST['login_pwd']);
				}
				else return false;
			}
			else {
				// non vérification du reCaptcha
				if (isset($captcha_count)) $captcha_count += 1;
				else $captcha_count = 1;
				if ($captcha_count > MAXCOUNTLOGIN) {
					$_SESSION[SHORTNAME.'_user_error'] = 'Authentification impossible, contactez un administrateur du site';
					return false;
				}
				else {
					$_SESSION[SHORTNAME.'_user_error'] = 'Erreur reCAPTCHA (encore ' . (MAXCOUNTLOGIN + 1 - $captcha_count) . ' essai' . ((MAXCOUNTLOGIN + 1 - $captcha_count) > 1 ? 's' : '') . ') : confirmez que vous n’êtes pas un robot en cochant la case.';
					$_SESSION['reCaptcha-wrong-count'][session_id()] = $captcha_count;
				}
				return false;
			}
